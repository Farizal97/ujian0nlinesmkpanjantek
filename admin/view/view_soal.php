...
open_form("modal_print", "return print_data()");
   $qujian = mysqli_query($mysqli, "SELECT * FROM ujian WHERE id_ujian='$_GET[ujian]' ");
   $list = array();
  while($ru = mysqli_fetch_array($qujian)){
      $list[] = array($ru['id_ujian'], $ru['ujian']);
   }
   create_combobox("Ujian", "ujian_print", $list, 4, "", "required");
