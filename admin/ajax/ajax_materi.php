...
if($_GET['action'] == "table_data"){
   $query = mysqli_query($mysqli, "SELECT * FROM ujian WHERE id_guru='$user' ORDER BY id_ujian DESC");
   $data = array();
   $no = 1;
...
elseif($_GET['action'] == "import"){
	...
	if($extensi != ".pdf"){
		...
		if(move_uploaded_file($_FILES['file']['tmp_name'], "$path/$filename")){
			mysqli_query($mysqli, "UPDATE ujian SET materi='$filename' WHERE id_ujian='$_POST[ujian]'");
		}else{
