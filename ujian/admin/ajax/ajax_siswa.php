...
elseif($_GET['action'] == "import"){
   ...
   if($extensi != ".xls"){
      ...
      for($i=2; $i<=$jdata; $i++){		
         $nus = addslashes(str_replace(" ", "", $data->val($i,2)));
         $nama	= addslashes($data->val($i,3));
				
         $cek = mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM siswa WHERE nus='$nus'"));
         if($cek > 0){
            mysqli_query($mysqli, "UPDATE siswa SET nama='$nama', id_kelas='$_POST[kelas_import]' WHERE  nus='$nus'");
         }else{
            $pass = md5(substr(md5($nus),0,5));
