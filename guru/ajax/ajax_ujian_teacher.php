...
//Membuat tombol kelas untuk melihat nilai	
   ...
   while($rk = mysqli_fetch_array($qkelas)){
      $jml = mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM nilai t1, siswa t2 WHERE t1.id_ujian='$rk[id_ujian]' AND t1.id_siswa=t2.id_siswa AND t2.id_kelas='$rk[id_kelas]'"));
